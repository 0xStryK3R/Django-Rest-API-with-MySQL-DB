...
def countries_list(request):
    if request.method == "GET":
        ...
        return JsonResponse(
            countries_serializer.data, safe=False, status=status.HTTP_200_OK
        )
...
def countries_detail(request, pk):
    try:
        ...
        return JsonResponse(
            {"message": "The country does not exists"}, status=status.HTTP_404_NOT_FOUND
        )
    ...
    if request.method == "GET":
        ...
        if countries_serializer.is_valid():
            countries_serializer.save()
            return JsonResponse(
                countries_serializer.data, status=status.HTTP_202_ACCEPTED
            )
        return JsonResponse(
            countries_serializer.errors, status=status.HTTP_400_BAD_REQUEST
        )
